...
## 2. Readonly
Constructs a type where all properties are set to `readonly`. 
...
## 3. Exclude
Constructs a type by excluding from union type `T` all union members that are assignable to `U`.
### Usage
...
## 5. Omit
[Omit\<T\>](https://www.typescriptlang.org/docs/handbook/utility-types.html#https://www.typescriptlang.org/docs/handbook/utility-types.html#omittype-keys) Constructs a type by picking all properties from `T` and then removing keys `K`.
...
(Technically not a built-in utility type, but a common challenge.)
Extracts the first element type from an array or tuple `T`.
### Usage
```typescript
type arr1 = ['a', 'b', 'c']
type arr2 = [3, 2, 1]
